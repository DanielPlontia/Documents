 ...
 * @Date       : 2014
 */
package subscriber.template.sensor.drone;
...
public class EBSubscriberManagerTemplate extends EBAbstractSubscriberManager {
     ...
     */
    public void init() {
        /* 현재 예제 프로그램인 '빌딩 보안시스템'의 코드가 삽입되어 있다.  해당 코드는 Subscriber들 중 하나인 Drone Sensor를 구동하는 코드이다.*/
        SensorSubscriber droneSubscriber = new SensorSubscriber(ComponentType.DRONE, null, EBConstants.RMI_PORT);
        ObserverForSensor observerForDrone = new ObserverForSensor(droneSubscriber.getSensorAlarm(), droneSubscriber); 
        
        addUserSubscriber(droneSubscriber);
        addUserObserver(observerForDrone);
    }
